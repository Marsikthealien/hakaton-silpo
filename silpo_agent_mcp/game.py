...
# Нагороди за рівні
# ---------------------------------------------------------------------------
# Що вищий рівень — то краща нагорода. Кожні 5 рівнів.
#
# ВАЖЛИВО: ми НЕ видаємо купонів і не маємо на це права. Нижче — концепт
# нагороди: рівень, назва й тип. Замість промокоду віддається позначка
# «КОНЦЕПТ·Lxx», яка навмисне не схожа на код і в «Сільпо» нічим не є.
# Видача — виключно на боці мережі: у застосунку для цього вже є
# /v1/loyalty/my/create-coupon-batch і /v1/loyalty/my/create-coupon-by-link,
# у MCP «Сільпо» `silpo_get_promo_codes` уміє лише читати. Саме tool на видачу
# нагороди за прогрес ми й пропонуємо — рішення про неї приймає «Сільпо».
# Перелік редагується з «Під капотом» (silpo_agent_mcp/demo.py).
...
def _code_for(level: int) -> str:
    """Позначка концепту, а не промокод.

    Навмисно не схожа на код «Сільпо»: нагороду за прогрес видає мережа,
    ми лише показуємо, як це виглядало б.
    """
    return f"КОНЦЕПТ·L{level:02d}"
...
async def claim_level_reward(level: int | None = None) -> dict:
    """ЗАПРОПОНОВАНИЙ tool. Позначити нагороду за досягнутий рівень.

    Купона тут немає і бути не може: видавати знижки може лише «Сільпо».
    Ми показуємо концепт — який рівень, яка нагорода, коли відкрилась, —
    і саме tool на видачу нагороди за прогрес і є те, чого бракує.
    """
    ...
    return {"granted": reward, "level": state["level"], "proposed": True,
            "note": ("Це концепт, а не купон. Видавати знижки може лише «Сільпо»: "
                     "у застосунку для цього є /v1/loyalty/my/create-coupon-batch, "
                     "у MCP `silpo_get_promo_codes` уміє лише читати."),
            "spec": SPEC_URL}
